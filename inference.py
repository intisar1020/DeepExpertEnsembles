...
def load_experts(num_classes, list_of_index=[None], pretrained=True):
    ...
    for loi in list_of_index:
        ...
        if pretrained:
            ...
            try:
                chk = torch.load(f"{chk_path}.pth")
            except FileNotFoundError:
                chk = torch.load(f"{chk_path}.pth.tar")
...
def inference_with_experts_and_routers(test_loader, experts, router, topk=2, total_data=10000, topk_experts=None):
    router.eval()
    experts_on_stack = []
    expert_count = {}
    for k, v in experts.items():
...
def main():
    ...
    if args.ensemble_inference:
        ...
        inference_with_experts_and_routers(
            test_loader_single, msnet, router, total_data=total_data, topk=args.topk, topk_experts=None
        )
